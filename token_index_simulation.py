...
# Lists to store cumulative daily percentage change for each symbol
cumulative_changes = []
...
for symbol in symbols:
    ...
    if protocol_address:
        ...
        if timestamps and prices:
            ...
            # Append cumulative percentage change to the list
...
# Calculate average cumulative percentage change across all symbols
avg_cumulative_change = np.mean(cumulative_changes, axis=0)
timestamps = timestamps[1:]  # Remove first timestamp since there's no change for it

# Plotting the line chart for average cumulative percentage change
plt.plot(timestamps, avg_cumulative_change, label='Average')

# Adjust x-axis limits based on the chosen period
plt.xlim(min_date, max_date)

plt.title(f'Average Cumulative Percentage Change of Protocols ({period_input})')
plt.xlabel('Date')
plt.ylabel('Average Cumulative Percentage Change (%)')
